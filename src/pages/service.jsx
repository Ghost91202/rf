...
const service = () => {
  ...
  useEffect(() => {
    ...
    const donateSection = donateSectionRef.current;

    const handleScroll = () => {
      const scrollPosition = window.scrollY + window.innerWidth;

      if (scrollPosition > textContainer.offsetLeft) {
        textContainer.classList.add('slide-in');
      }

      if (scrollPosition > donateSection.offsetLeft) {
        donateSection.classList.add('slide-in-fast');
      }
    };
  ...
  <div className='w-full sm:w-3/4 h-auto lg:h-auto'>
      <div ref={textContainerRef} className="text-container p-10 md:p-20 lg:p-32 font-semibold text-balance">
        <h2 className="mb-4 text-2xl lg:text-6xl xl:pt-20 tracking-tight font-bold text-black">
          Welcome to <span className='text-orange-600'>Rahil Foundation</span>
            </h2>
            <Slider Name={  <p className="mb-4 xl:w-3/4">
          Where we embark on a transformative journey towards cyber literacy. At Rahil, we recognize the pivotal role of digital understanding in today's interconnected world. Our mission is to empower individuals with the knowledge and skills to navigate the digital landscape safely and responsibly.
        </p>}/>


            <br />
            <Slider Name2={<p className='xl:w-3/4'>
    ...
    <div className='bg-orange-500 w-full sm:w-96 text-white p-10'>
       <div ref={donateSectionRef} className='flex flex-col border-b-2 pb-4 pt-5 gap-3 donate-section'>
        ...
        <Link className='font-semibold flex items-center'>Get started <TiArrowForwardOutline className='font-bold text-xl' /></Link>
      </div>
